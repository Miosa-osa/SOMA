...
The vsock model accepts one stream connection at a time on the fixed control port 5001, which no earlier branch had fixed, so the guest agent and Generation manifest must adopt `SOMA_CONTROL_PORT`.
