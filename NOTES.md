...
The claim registry binds each `OperationId` to a request fingerprint: a replay with the same fingerprint returns the identical `ClaimOutcome` for the life of the pool, a changed fingerprint is a typed `OperationConflict`, a concurrent replay waits at most the claim deadline for the in-flight attempt, and an exhausted pool answers with a typed `Exhausted` carrying the occupancy instead of queueing; an operator may instead select inline construction, which is labelled `OnDemand` so it never mixes with prepared-worker results.
