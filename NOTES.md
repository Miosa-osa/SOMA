...
Windows rejects the ledger's Unix directory-fsync operation with `PermissionDenied`; record files are flushed everywhere, while parent-directory metadata is flushed only on Unix production hosts.
