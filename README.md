...
| Ubuntu 24.04 and 26.04 x86_64 | Native CI | KVM capability probe; cold-boots a compiled Generation, authenticated guest agent, first bounded command; restores a captured snapshot to an authenticated command (test-only) | A compiled busybox Generation booted on a real host with the five virtio-mmio devices, the static guest agent authenticated over vsock, one bounded command returned its bytes and exit status, and cleanup was proven; a `node:22` Generation was then captured at its repair point and restored repeatedly into independent authenticated Instances; no network egress, jail, prepared workers, or sandbox lifecycle process yet |
