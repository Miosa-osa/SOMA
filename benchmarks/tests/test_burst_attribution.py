...
class BreakdownTests(unittest.TestCase):
    def test_reasons_are_counted_with_the_details_that_explain_them(self) -> None:
        rows = failure_breakdown(
            [
                [{"reason": "launch_process_failed", "operation": "launch", "detail": "exit=76"}],
                [{"reason": "launch_process_failed", "operation": "launch", "detail": "exit=76"}],
                [{"reason": "cleanup_failed", "operation": "destroy", "detail": "exit=69"}],
            ]
        )
        ...
        self.assertEqual(
            breakdown_lines(rows)[0], "2x launch_process_failed at launch"
        )
...
class AttributionContractTests(unittest.TestCase):
    def test_a_failure_that_names_no_cause_is_refused(self) -> None:
        ...
        sample = sample_record(
            ...
            1,
            successful=False,
            failures=[{"reason": "launch_process_failed", "operation": "launch", "detail": ""}],
        )
        completion = {"attempted": 1, "failure_breakdown": []}
