...
| In-guest secret delivery: a per-Instance value placed over the authenticated session at an owner-read-only mode | Component-tested | `crates/soma-guest` placement, redaction, and refusal tests, `crates/soma-guest-agent` mode tests, and [ADR 0037](adr/0037-in-guest-secret-delivery.md). The placement is wired into the KVM launch path and fails the launch closed, but no credential source resolves a `secret://` reference into a value yet, so it runs over an empty set; the host-side mediator of the second delivery mode does not exist |
...
| Real guest networking: virtio-net attach, TAP transfer, proxy | Designed | [Linux network profile v1](research/linux-network-profile-v1.md) |
