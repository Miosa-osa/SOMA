...
| Capture and restore on the current per-Instance authority design | Live-proved at `5d71524` | [x86_64 capture and restore on the per-Instance authority design](evidence/2026-08-30-x86_64-snapshot-restore-current-authority.md); six live tests, and an object scan showing no Instance responder identity and no decodable launch page in `memory.raw`, `overlay.raw`, or `state.somasnap`. Closes finding P1.5 of [the re-audit](reviews/2026-08-29-implementation-reaudit.md) |
