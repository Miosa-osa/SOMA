...
| HTTP sandbox lifecycle service | Component-tested | `crates/soma-api` tests. Create, get, run command, stop, and destroy are served over HTTP against the portable facade. Listing sandboxes and every guest filesystem operation refuse with a typed capability error, because the durable state store cannot enumerate, and no backend or engine method reaches the guest filesystem operations the guest protocol implements. Tenant identity is an unfinished seam that refuses every request without it |
