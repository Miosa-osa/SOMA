...
4. EROFS image digest, byte size, UUID, format profile, formatter executable digest, formatter revision, and builder image digest.
...
Initramfs layout v2 adds the `/dev/console` and `/dev/null` nodes PID 1 needs before devtmpfs is mounted and the Generation-scoped responder private key at `/etc/soma/responder.key`, supplied as a fifth machine input; the matching public key is not yet bound into the manifest and is carried by the launcher.
