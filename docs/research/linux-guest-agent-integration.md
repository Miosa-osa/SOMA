...
The VMM creates one fresh 4 KiB launch page containing magic, schema, GenerationId, InstanceId, OperationId, nonce, Noise PSK material, guest entropy seed, assigned vsock CID, network generation, time sample, the fresh per-Instance responder static secret decided by [ADR 0024](../adr/0024-per-instance-guest-responder-authority.md), and a digest over the page.
