...
This result proves that SOMA revision `18c8014fc1e69df7ef66f3ae09f599df87adee20` plus the `kernel/` pipeline on branch `feat/pinned-x86-64-kernel` builds Linux `v6.12.107` into an uncompressed x86_64 ELF `vmlinux` that carries `XEN_ELFNOTE_PHYS32_ENTRY`, links every `PT_LOAD` segment at or above `0x01000000`, and reproduces byte for byte across two consecutive builds on one host.
...
- SOMA Git revision: `18c8014fc1e69df7ef66f3ae09f599df87adee20` (worktree branch `feat/pinned-x86-64-kernel`, kernel pipeline uncommitted at build time).
...
- Pinned config `kernel/config-x86_64-soma-v1` SHA-256: `39f38021c69bfce9963926bb19b5c30cbd437d3902ab31959054af5153c8fd74`.
- Config fragment `kernel/soma-v1.fragment` SHA-256: `53818976f112f74730b7e1b168724ec7e12da4e41c94671fd718a08a1df2ee59`.
- Required-symbol list `kernel/required-config.txt` SHA-256: `33416621de88392ac550f73541c88dee22ac6ddb01e465c4177d735ec141b62a`.
...
| --- | --- | ---: | ---: | ---: | --- | ---: |
| 1 | none (1615 pinned symbols unchanged, 59 required symbols hold) | 48.8 s | 58 s | 58.84 s | `cf071d83d5461a0b739a5c361825f994a528e0b5bee1b9b78350e5f07b22755c` | 21,530,056 bytes |
| 2 | none (1615 pinned symbols unchanged, 59 required symbols hold) | 48.1 s | 58 s | 58.72 s | `cf071d83d5461a0b739a5c361825f994a528e0b5bee1b9b78350e5f07b22755c` | 21,530,056 bytes |

`cmp` reported the two `vmlinux` files identical.
Both post-`olddefconfig` `.config` files had SHA-256 `39f38021c69bfce9963926bb19b5c30cbd437d3902ab31959054af5153c8fd74`, identical to the pinned config.
The two manifests differed in exactly one field, `builder.image_id` (`sha256:3067c203...` versus `sha256:c58a9cfa...`), because `docker build` re-executed the `apt-get` layer and produced a new image identity each time while installing identical package versions.
The output bytes did not depend on that identity.

Loaded footprint reported by `size`: text 11,037,307 bytes, data 3,832,456 bytes, bss 1,802,244 bytes, total 16,672,007 bytes.
...
verify-pvh: OK
  ...
  PT_LOAD paddr=0x1000000 vaddr=0xffffffff81000000 filesz=10301772 memsz=10301772 R-E
...
## Known gap discovered after the build

The built configuration has `CONFIG_DEVMEM=n`, so the guest agent cannot read the SOMA launch page through `/dev/mem` with this artifact.
`kernel/soma-v1.fragment` was updated afterwards to request `CONFIG_DEVMEM=y` and `CONFIG_STRICT_DEVMEM=n`, but no rebuild was performed and this document describes only the `CONFIG_DEVMEM=n` kernel.
A regenerated config, a rebuild, and a new evidence record are required before the kernel is a valid Generation input.
