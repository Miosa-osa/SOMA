...
Snapshot capture and restore are implemented in `soma-kvm` and are live-proved at `5d71524` on the current per-Instance authority design; [the `7c1127d` run](../evidence/2026-08-29-x86_64-snapshot-restore.md) is retained as historical because it predates initramfs layout v3, launch-page schema 3, and the authenticated readiness receipt.
...
The same path after a snapshot restore is live-proved at `5d71524` in [the capture and restore on the per-Instance authority design](../evidence/2026-08-30-x86_64-snapshot-restore-current-authority.md).
