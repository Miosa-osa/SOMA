...
Snapshot capture and restore are implemented in `soma-kvm` and component-tested on schema 2.
The `5d71524` live run is historical because ADR 0032 changed the snapshot and Generation identities and added the captured overlay to the certified binding.
The workspace still does not contain Generation certification, a Generation built from a Template Lock, the jail launcher behind the host allocator, a production network attachment to the Machine, the KVM adapter ticket #13 specifies, or remote transport. A cold-boot slice of that adapter exists as of `08e4d45` and is live-proved for one sandbox: it resolves a prepared Candidate, cold boots, runs one command, and cleans up, without snapshot restore, capacity admission, prepared workers, or networking.
...
It also owns the platform-neutral snapshot format v2 codec under `snapshot/`: the `SOMASNP` manifest, bounded digest-covered sections, SOMA-owned byte layouts for every x86_64 KVM state group, per-device state, the memory-object descriptor, the fail-closed compatibility check, and the capture and restore ordering contracts.
`x86_64/snapshot/` is the live half of that format: it pauses a running machine at the guest agent's disconnected repair point, proves the quiesce preconditions, reads KVM and device state in the certified order, publishes the memory object, the captured overlay, and the state manifest, and restores that snapshot into a fresh Instance through the twelve-step machine-contract order.
The code is component-tested on schema 2, while the `5d71524` live run proves the preceding schema and is historical.
...
`snapshot/` owns the snapshot format v2 encoding and validation policy from `docs/research/snapshot-format-v2.md`.
...
`manifest.rs` owns the `SOMASNP\0` header, schema version, architecture, page size, `CandidateId`, contract digests, host-profile requirements, memory descriptor, machine shape, and the canonical ascending section sequence.
...
The `generation/` modules implement immutable artifact compilation and snapshot certification for x86_64: one `TemplateRevision` plus one `NormalizedRootfs` become a Candidate containing an EROFS root, sterile ext4 overlay templates, a verified kernel, and a deterministic initramfs.
After boot and capture, certification binds the exact memory, overlay, and state artifacts into a canonical `SOMAGEN` version 2 manifest whose SHA-256 is the `GenerationId`.
...
`CompiledCandidate` carries a published Candidate manifest, its `CandidateId`, formatter and checker evidence, and the remaining boot-and-capture phase.
`verify_generation` accepts only a ready `SOMAGEN` manifest, re-verifies every artifact, validates the snapshot's internal bindings, and reports the Generation launchable only after those checks pass.
...
`pool/key.rs` is the exact [`PoolKey`] digest; nothing is rounded or widened so a sterile worker can never be prepared for a different contract, and `overlay.template_digest` names the storage crate's sterile ext4 template rather than the user-authored Template of the glossary.
...
`pool/replenish.rs`, `pool/replenish/background.rs`, and `pool/backpressure.rs` bound construction by concurrency, deadline, target, and maximum and return typed `Exhausted` and `Overloaded` results; every pass reaps the construction threads that finished, so nothing accumulates one join handle per construction ever started.
`pool/ledger.rs`, `pool/ledger/record.rs`, and `pool/ledger/fold.rs` are the durable ledger of create-exclusive, checksummed, file-and-directory-synced records and the projection that fails closed on any record that skips a phase of the transition table, moves the lease generation that only a claim bumps, or claims a worker that was already assigned; the fresh per-Instance references are recorded before the first authority frame and refreshed by every transfer record, so a crash mid-transfer releases the leased head and the network bundle at the next reconciliation.
`pool/reconcile.rs` treats every nonterminal entry without a live slot as suspect after a restart, probes the launcher and the brokers, terminates or releases, retains a live running Instance, and gates replenishment until it has run; passes are serialized and the slot table refuses a worker it already holds, so no Instance is adopted twice.
...
`admission/` is the visual atlas capacity model: a certified host profile with host reserve, labelled measured per-VM overhead, per-dimension limits, and per-class overcommit ratios, which only [`HostProfile::validate`] and [`MachineShape::validate`] can produce so no unvalidated profile or shape reaches the arithmetic; checked demand and division arithmetic that answers an overflow with `Gate::Arithmetic` instead of aborting; an atomic multi-dimension reservation that rolls back on the first refusing gate; a typed rejection naming the gate and its numbers; explicit guaranteed and elastic memory classes; a single-node NUMA placement hook; and the capacity ladder estimate.
`protocol.rs` and `daemon.rs` are the bounded typed claim, release, inspect, and reconcile protocol and the single-threaded `SOCK_SEQPACKET` skeleton over one pool; the daemon does not authenticate its peer, answers a replayed claim with the launch page it delivered or a typed terminal failure when the worker is gone, and starts only with the explicitly requested in-process development launcher, and `bin/linux/host.rs` builds the certified host profile and Machine shape it admits against from explicit operator inventory arguments.
