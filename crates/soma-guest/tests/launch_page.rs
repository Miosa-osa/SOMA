use soma_guest::{
    Error, GuestLaunchMaterial, HostLaunchMaterial, LAUNCH_PAGE_SIZE, SessionBinding,
};
...
fn fresh_launch_material_crosses_one_page_for_one_instance() {
    ...
    let instance = [2; 16];
    let operation = [3; 16];
    let host = HostLaunchMaterial::generate(generation, instance, operation)
...
fn every_malformed_launch_page_is_rejected_and_fully_wiped() {
    let host = HostLaunchMaterial::generate([1; 32], [2; 16], [3; 16])
        .expect("operating-system launch material");
    ...
    for range in [0..16, 20..52, 52..68, 68..84, 84..116, 116..148, 148..212] {
        let mut page = canonical;
        page[range].fill(0);
        malformed.push(page);
    }
    for offset in [16, 18, 212] {
        let mut page = canonical;
        page[offset] ^= 1;
        malformed.push(page);
    }
