...
impl Claimed {
    ...
    ///
    /// `activate` spends the capability the repaired session mints, exactly as it does for a
    /// machine a Launch restored itself: a claimed machine with a leased network is not Ready
    /// until the broker has enabled forwarding and the link gate has opened.
    ///
    /// # Errors
    ///
    ...
    pub(in crate::backend) fn assign(
        ...
        match session.assign(assignment, activate) {
            Ok(()) => {
