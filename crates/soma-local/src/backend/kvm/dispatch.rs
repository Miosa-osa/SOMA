//! The five Backend operations, and which process performs each one.
//!
...
impl KvmBackend {
    pub(in crate::backend) fn launch(
        ...
        Ok(LaunchObservation::new(
            ...
            // LaunchEnforced would claim a binding no gate produced.
            DigestBinding::ObservedOnly,
            effective_shape(launched.memory_mib),
            launched.network,
            // The launch stamp is on the clock of whichever process built the machine, so a
            // hosted launch reports it after this operation was admitted here and never after
            // the moment this process saw the machine become ready.
            LaunchTimes::new(
                admitted,
                admitted.saturating_add(launched.launched_ns).min(ready),
                ready,
            ),
        ))
    }
    ...
    pub(in crate::backend) fn execute(
        ...
        let outcome = match self.hosted_directory() {
            None => GuestCommand::new(parts.program, parts.arguments, parts.timeout_ms, parts.max_output_bytes)
