...
//!
//! It binds its socket before anything exists, so an Instance is addressable from the moment it
...
/// How long a host with a machine nobody is addressing stays alive.
///
/// A client that crashed between launch and destroy would otherwise leave a machine running for
/// as long as this host does, which is forever. The ceiling is generous enough that an idle
/// agent session is not cut off and finite enough that an abandoned machine is not a permanent
/// leak.
const IDLE_CEILING: Duration = Duration::from_secs(1800);
...
///
/// The prepared entry is found here rather than handed over, so what launches is what a prepared
/// entry in this host's store claims for the reference, verified by the same read the one-shot
/// path performs.
fn launch(
    backend: &mut KvmBackend,
    request: &LaunchWire,
) -> Result<super::Launched, BackendFailureKind> {
...
fn answer_until_released(
    ...
    // Nothing is coming, so this host releases what it still holds rather than keeping a machine
    // alive that no process can reach.
    let _ignored = backend.cleanup_resident(instance);
    RELEASED
}
...
fn perform(backend: &mut KvmBackend, call: Call) -> Answer {
    match call {
        Call::Execute {
            ...
            let Ok(command) =
                GuestCommand::new(program, arguments, timeout_ms, max_output_bytes)
            else {
        ...
        // Both terminal calls are decided by the caller of this function.
...
///
/// The shutdown is requested over the host's own socket rather than by touching the machine from
/// another thread, so the release still happens on the one thread that owns it.
