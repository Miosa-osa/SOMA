...
//! arrives. What remains on the request path is the private disk head and the context
//! identifier, which are the two authorities that belong to one Instance.
//!
...
use soma_kvm::x86_64::{SnapshotPaths, SterileRequest, restore_sterile};

use super::session::{Request, Response, SessionError};
use super::worker::{drive_restored, report};
...
/// head, an Instance, an operation, a context identifier, or any launch material, because a
/// worker holding any of those before it is claimed would not be sterile.
...
pub(super) struct Assignment {
    ...
    /// The network identity written into the launch page.
...
pub(super) fn serve(spec: SterileSpec, requests: &Receiver<Request>, responses: &Sender<Response>) {
    ...
    // rather than returning a half-assigned one to anybody.
    let Ok(mut restored) = sterile.assign(overlay, guest_cid, None) else {
        let _ignored = responses.send(Response::Failed(SessionError::Create));
        return;
    };
