...
//! Neither changes what happens here, so per-Instance identity, the sterile assignment, and the
//! Noise session are established exactly once either way.
...
impl KvmBackend {
    ...
    pub(super) fn execute_resident(
    ...
    ) -> Result<(CommandStatus, Vec<u8>, Vec<u8>), BackendFailureKind> {
        ...
        let completed = live
            ...
            .map_err(super::start::failure_kind)?;
    ...
    ///
    /// Cleanup is idempotent, but an Instance this Backend never owned is a different fact from
    /// one it owned and released. Reporting resources complete here would claim to have released
    /// resources this process never held, so every disposition stays `NotOwned` and only the
    /// Host Runtime's ownership of the identity is ended.
    pub(super) fn release_unowned(
        &mut self,
        instance: &InstanceId,
    ) -> Result<CleanupEvidence, BackendFailureKind> {
        self.ownership.release(instance)?;
        Ok(super::lifecycle::lookup::not_owned_evidence())
    }
}
