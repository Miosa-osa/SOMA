...
pub use virtio::{
    ...
    BlockCounters, BlockDevice, BlockOp, BlockRequest, BlockRole, BlockState, CID_ANY,
    ChainHandler, ChainLimits, ChainSegment, ChainViolation, ConfigAccessError, Credit,
    CreditError, DESCRIPTOR_SIZE, Descriptor, DescriptorChain, DeviceFault, DeviceStateError,
    DeviceStatus, EntropyBackend, EntropyError, FrameError, GuestAddress, GuestMemory,
    GuestMemoryError, GuestValue, HOST_BUF_ALLOC, HOST_CID, HOST_TX_BUFFER, HostEndpoint,
    INTERRUPT_CONFIG_CHANGE, INTERRUPT_USED_BUFFER, LOOPBACK_QUEUE_LIMIT, LayoutViolation,
    LoopbackBackend, LoopbackHandle, MAX_CONFIG_LEN, MAX_ENTROPY_REQUEST, MAX_FRAME_LEN,
    MAX_OUTBOUND_PACKETS, MAX_PAYLOAD_LEN, MAX_PENDING_EVENTS, MAX_QUEUE_SIZE, MAX_QUEUES,
    MAX_REQUEST_BYTES, MAX_RX_CHAIN_BYTES, MIN_FRAME_LEN, MIN_GUEST_CID, MMIO_PAGE_SIZE,
    MemoryBackend, MmioTransport, NET_CONFIG_LEN, NET_FEATURES, NET_QUEUE_MAX, NET_RX_QUEUE,
    NET_STATE_LEN, NET_STATE_VERSION, NET_TX_QUEUE, NetBackend, NetBackendError, NetCounters,
    NetDevice, NetState, PacketError, QUEUE_STATE_LEN, Queue, QueueLayout, QueueState,
    QueueStateError, QueueViolation, QueueViolationCounters, QueueViolationKind,
    REQUEST_HEADER_LEN, RNG_FEATURES, RNG_QUEUE_MAX, RNG_STATE_LEN, RNG_STATE_VERSION,
    RegionLayoutError, Register, RequestError, RequestLimits, RestoreError, RngCounters, RngDevice,
    RngState, SECTOR_SIZE, SOMA_CONTROL_PORT, SOMA_VENDOR_ID, ServiceError, ServiceReport,
    StatusViolation, StatusWrite, TRANSPORT_STATE_HEADER_LEN, TapBackend, TransportConfigError,
    ...
    deliver_vsock_rx, parse_request, parse_tx, read_readable, service_queue, validate_size,
    validate_tx, walk_chain, write_writable,
};
