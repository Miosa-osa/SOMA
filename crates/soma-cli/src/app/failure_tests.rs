...
use soma::{BackendFailureKind, FailurePhase, ManagedFailure, RunFailureKind, StateStoreFailureKind};
...
fn an_unavailable_local_backend_is_not_retryable() {
    ...
    let error = execution.response.error().expect("a refusal carries an error");
...
fn only_a_clearing_state_store_condition_is_retryable() {
    ...
    for kind in permanent {
        ...
        let execution = managed_failure("machine.launch", instance(), &ManagedFailure::StateStore(kind));
        let error = execution.response.error().expect("a refusal carries an error");
...
fn the_backends_that_host_a_machine_only_in_this_process_are_named() {
    ...
    assert_eq!(
        soma_local::machine_hosting(soma::BackendKind::DockerContainer),
