...
const OPERATION_ID: &str = "44444444444444444444444444444444";
...
impl ToolRuntime for FakeRuntime {
    async fn invoke(&self, request: RuntimeRequest) -> Result<RuntimeResponse, RuntimeFailure> {
        ...
        Ok(RuntimeResponse::Stop(MachineResult::new(
...
#[tokio::test]
async fn stop_carries_retry_identity_and_cleanup_receipt() {
    ...
    let result = client
        .call_tool(CallToolRequestParams::new("soma_stop").with_arguments(arguments))
        .await
        .expect("tool response");

    assert_eq!(result.is_error, Some(false));
    let structured = result.structured_content.expect("structured result");
    assert_eq!(structured["operation"], "stop");
    assert_eq!(structured["operation_id"], OPERATION_ID);
    assert_eq!(structured["result"]["state"], "stopped");
    assert_eq!(structured["receipt"]["cleanup"], "complete");

    {
        let requests = requests.lock().expect("request ledger");
        let RuntimeRequest::Stop(request) = requests.first().expect("stop request") else {
