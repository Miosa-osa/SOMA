//! A terminal the terminal routes can be proved against without KVM.
//!
//! It holds the one session the protocol allows and echoes what was typed at it, which is enough
//! to prove that each route reaches the facade with the operation it named and renders the answer
//! it got back.
...
/// Answers one operation against the one session.
