...
readonly IMAGE="$1"
STORE_DIR="$(mkdir -p "$2" && cd "$2" && pwd -P)"; readonly STORE_DIR
FS_TOOLS="$(cd "$3" && pwd -P)"; readonly FS_TOOLS
readonly MEM_MIB="${4:-1024}"
readonly DISK_MIB="${5:-10240}"
...
export_oci() {
    ...
    skopeo copy --override-os linux --override-arch amd64 \
        "docker://docker.io/library/$IMAGE" "oci:$LAYOUT:$safe"
